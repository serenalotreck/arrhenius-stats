...
#source("Shiu_lab/arrhenius-stats/arrhenius_stats.R", chdir = TRUE)
## NOTE: Need to run the above source command on the command line, followed by 
## testthat::test_file("Shiu_lab/arrhenius-stats/test_arrhenius_stats.R") 
...
test_that('No NA or missing measurement, 5 days', {
  ...
  LNGPP <- c(0.61, 0.48, 0.69, 0.57, 1.28)
  LNER <- c(1.27, 0.67, 0.55, 0.37, 0.24)
  input <- data.frame(SITE, DATE, STANDTEMP, LNGPP, LNER)
  ...
  row1_lm_GPP <- lm(LNGPP~STANDTEMP, data = input[1:2,])
  row1_lm_ER <- lm(LNER~STANDTEMP, data = input[1:2,])
  ...
  row3_lm_GPP <- lm(LNGPP~STANDTEMP, data = input[3:5,])
  row3_lm_ER <- lm(LNER~STANDTEMP, data = input[3:5,])
  ...
  colnames(output) <- c("slope_LNGPP", "yint_LNGPP", "Fstatistic_LNGPP", 
                        "dF_LNGPP", "p_LNGPP", 
...
test_that('Missing LNGPP or ER', {
  ...
  DATE <- as.Date(DATE, format = "%m/%d/%y")
  STANDTEMP <- c(-0.34, -0.70, -0.24, -0.42, 0.48)
  LNGPP <- c(0.61, 0.48, 0.69, 0.57, NA)
  LNER <- c(1.27, 0.67, NA, 0.37, 0.24)
  input <- data.frame(SITE, DATE, STANDTEMP, LNGPP, LNER)
  ...
  row2_stats <- row1_stats
  row3_lm_GPP <- lm(LNGPP~STANDTEMP, data = input[3:4,])
  row3_stats <- c(summary(row3_lm_GPP)$coefficients["STANDTEMP", "Estimate"],
                  summary(row3_lm_GPP)$coefficients["(Intercept)", "Estimate"],
                  summary(row3_lm_GPP)$fstatistic["value"],
                  summary(row3_lm_GPP)$fstatistic["numdf"],
                  suppressWarnings(anova(row3_lm_GPP)$'Pr(>F)'[1]),
                  NA, NA, NA, NA, NA)
  row4_lm_GPP <- lm(LNGPP~STANDTEMP, data = input[3:4,])
  row4_lm_ER <- lm(LNER~STANDTEMP, data = input[4:5,])
  ...
  row5_lm_ER <- lm(LNER~STANDTEMP, data = input[4:5,])
...
test_that('Site IDs are different', {
  ...
  DATE <- as.Date(DATE, format = "%m/%d/%y")
  STANDTEMP <- c(-0.34, -0.70, -0.24, -0.42, 0.48)
  LNGPP <- c(0.61, 0.48, 0.69, 0.57, 0.78)
  LNER <- c(1.27, 0.67, 0.45, 0.37, 0.24)
  input <- data.frame(SITE, DATE, STANDTEMP, LNGPP, LNER)
  ...
  row3_lm_GPP <- lm(LNGPP~STANDTEMP, data = input[c(3,5),])
  row3_lm_ER <- lm(LNER~STANDTEMP, data = input[c(3,5),])
  row3_stats <- c(summary(row3_lm_GPP)$coefficients["STANDTEMP", "Estimate"],
                  summary(row3_lm_GPP)$coefficients["(Intercept)", "Estimate"],
                  summary(row3_lm_GPP)$fstatistic["value"],
                  summary(row3_lm_GPP)$fstatistic["numdf"],
  ...
  row5_lm_GPP <- lm(LNGPP~STANDTEMP, data = input[c(3,5),])
  row5_lm_ER <- lm(LNER~STANDTEMP, data = input[c(3,5),])
...
test_that('Missing LNGPP and ER in multiple rows', {
  ...
  DATE <- as.Date(DATE, format = "%m/%d/%y")
  STANDTEMP <- c(-0.34, -0.70, -0.24, -0.42, 0.48)
  LNGPP <- c(0.61, 0.48, NA, 0.57, NA)
  LNER <- c(1.27, 0.67, NA, NA, 0.24)
  input <- data.frame(SITE, DATE, STANDTEMP, LNGPP, LNER)
...
test_that('Missing LNGPP in multiple rows', {
